sap.ui.define(["sap/m/MessageToast"], function (MessageToast) {
  ...
  return {
    getDocumentUrl: function (url) {
      const additionalUrl =
        this?.getEditFlow()?.getAppComponent()?._componentConfig?.url;
      if (additionalUrl) {
    ...
    onEarningFileUploadChange: async function (oEvent) {
      ...
      try {
        const fileUrl =
          "/odata/v4/earning-upload-srv/EarningFiles(ID=<ID>,IsActiveEntity=true)/content";
        ...
        //            oContext.setProperty("url", fileContent);
        oContext.setProperty(
          "url",
          fileUrl.replace("<ID>", oContext.getProperty("ID"))
        );
